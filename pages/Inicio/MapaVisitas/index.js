...
export default function SolicitaçãoDeVisitas() {
  ...
  const getVisitas = async () => {
    const res = await Api.getSolicitaçãoVisita();
    setInicioLead(false);
    setVisitas(res);
  };
  useEffect(() => {
    ...
    return (
      <Containerflet>
        <ContainerInfo>
          ...
          <TouchableOpacity onPress={handlePonto}>
            <Image
              style={{ width: 52, height: 32 }}
              source={require("../../../src/icons/rota.png")}
            />
          </TouchableOpacity>
        </ContainerInfo>
      </Containerflet>
  ...
  return (
    <Container>
      <Header>
        <HeaderTitulo>Solicitação de visitas</HeaderTitulo>
      </Header>
      <HeaderConteinerButoon>
        <HeaderButton onPress={() => navigation.navigate("Mapa")}>
